...
public class AmqpTests implements RabbitTestContainer {
	...
	public void testAmqpInboundGatewayFlow() {
		...
		assertThat(TestUtils.<AmqpTemplate>getPropertyValue(this.amqpInboundGateway, "amqpTemplate")).isSameAs(this.amqpTemplate);
	...
	void unitTestChannel() {
		...
		assertThat(TestUtils.<AmqpHeaderMapper>getPropertyValue(this.unitChannel, "inboundHeaderMapper")).isSameAs(this.mapperIn);
		assertThat(TestUtils.<AmqpHeaderMapper>getPropertyValue(this.unitChannel, "outboundHeaderMapper")).isSameAs(this.mapperOut);
