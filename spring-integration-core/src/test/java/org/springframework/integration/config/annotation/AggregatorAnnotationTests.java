...
public class AggregatorAnnotationTests {
	...
	public void testAnnotationWithCustomSettings() {
		...
		MessageHandler aggregator = this.getAggregator(context, endpointName);
		assertThat(TestUtils.<SimpleSequenceSizeReleaseStrategy>getPropertyValue(aggregator, "releaseStrategy")).isInstanceOf(SimpleSequenceSizeReleaseStrategy.class);
